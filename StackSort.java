...
public class StackSort {
    ...
    public static void main(String args[]) {
        ...
        // animatedStackExample(createArray(10, 0, 99));
        System.exit(0);
    }
    ...
    private static int[] doStackSort(int data[]) {
        ...
        StackInterface<Integer> lowerValues = new VectorStack<>();
        StackInterface<Integer> upperValues = new VectorStack<>();
        for(int index = 0; index < data.length; index++) {
        	int value = data[index];
        	while(!lowerValues.isEmpty() && value < lowerValues.peek()) {
        		upperValues.push(lowerValues.pop());
        	}
        	assert lowerValues.isEmpty() || value >= lowerValues.peek();
        	while(!upperValues.isEmpty() && value > upperValues.peek()) {
        		lowerValues.push(upperValues.pop());
        	}
        	assert upperValues.isEmpty() || value <= upperValues.peek();
